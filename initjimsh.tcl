...
set tcl::autohint_commands {array clock file package signal history zlib}

set tcl::autohint_cols {
	none {0}
	black {30}
	red {31}
	green {32}
...
set tcl::autohint_col $tcl::autohint_cols(lcyan)

# The default autohint implementation
proc tcl::autohint {string} {
	set result ""
	if {[llength $string] >= 2} {
		lassign $string cmd arg
		if {$cmd in $::tcl::autohint_commands || [info channel $cmd] ne ""} {
			catch {
				...
				if {[string match "Usage: $cmd *" $help]} {
					...
					set result [list $prefix$hint {*}$::tcl::autohint_col]
				}
			}
		}
	}
